...
const CODEX_PROFILE_HYDRATE_FLUSH_INTERVAL_MS = 500;
const CODEX_PROFILE_HYDRATE_CONCURRENCY = 5;
...
export const useCodexAccountStore = create<CodexAccountState>((set, get) => ({
  ...
  hydrateAccountProfilesIfNeeded: async (accountIds?: string[]) => {
    ...
    await Promise.all(
      Array.from(
        { length: Math.min(CODEX_PROFILE_HYDRATE_CONCURRENCY, candidates.length) },
        () => runNext(),
      ),
    );
